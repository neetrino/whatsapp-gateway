...
import { ProjectWebhookFanoutService } from '../../src/webhooks/project-webhook-fanout.service';
...
describe('WAHA inbound + project webhooks (e2e)', () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({ imports: [AppModule] })
      ...
      .useValue(prismaMock)
      .overrideProvider(ProjectWebhookFanoutService)
      .useValue({ deliver })
      .compile();
  ...
  it('skips RATE_LIMIT_SEND throttling', async () => {
    for (let i = 0; i < 80; i += 1) {
      const res = await postEvent({
        event: 'message',
        session: 'wa_m',
        payload: { id: `m${i}`, from: '37499111222@c.us', fromMe: false },
      });
      expect(res.status).toBe(200);
    }
